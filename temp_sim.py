...
@timeit
def tempeartureprofile(position=None, limit=45, delta_x=0.01, delta_t=0.1, time=360.0, length=0.4, **parameters):
	"""
	...
	:param position: (int) Parameter für die Ausgabe der Zeit an einer bestimmten Stelle [pos] = m {Standardwert None}
	...
	for time in tqdm.trange(time_steps):
		t1 = np.delete(temp_profile[time], 0)
		t2 = np.delete(t1, 0)
		t0 = np.resize(temp_profile[time], t2.size)
		t1 = np.resize(t1, t2.size)
		
		last = t0[-1] + (t0[-2]-t0[-1]) * A - B * (t0[-1]-t0[-2])
...
def find_time(temp_profile, limit, position, delta_t, delta_x, length):
	...
	time_cum_sum = np.insert(np.cumsum(np.full(len(temp_profile_T[0])-1, delta_t)),0,0) # Liste der Zeitpunkte
...
if __name__=="__main__":
	...
	time_gt = tempeartureprofile(**parameters)
	print(time_gt)
	
	print("| Starte Kalkulation mit Position |")
	position = tempeartureprofile(position =0.075, **parameters)
	print("An der Position 0.075 m wird die Grenztemperatur nach {} s erreicht".format(round(position,3)))
